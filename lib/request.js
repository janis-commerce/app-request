...
import { isNumber, isString, isObject } from "@janiscommerce/apps-helpers";
...
class Request {
  ...
  async get({
    ...
    try {
      ...
      if (!id || !isString(id)) throw new Error("id is not valid");
      ...
      crashlytics.recordError(
        error,
        `Error at GET:  service: ${service} namespace: ${namespace} id: ${id}`
      );
  ...
  async list({ headers = {}, queryParams = {}, namespace = "", service = "" }) {
    try {
      ...
      crashlytics.recordError(
        error,
        `Error at LIST:  service: ${service} namespace: ${namespace}`
      );
      return Promise.reject(error);
    }
  }
