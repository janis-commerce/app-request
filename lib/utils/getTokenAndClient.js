...
import { promiseWrapper, isObject } from "@janiscommerce/apps-helpers";

const getTokenAndClient = async () => {
  try {
    const [userInfo, errorUserInfo] = await promiseWrapper(getUserInfo());
    if (!isObject(userInfo) || errorUserInfo)
      throw new Error("error getting userInfo");

    const [accessToken, errorAccessToken] = await promiseWrapper(
      getAccessToken()
    );
