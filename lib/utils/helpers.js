 ...
 * @description function to make pathParams route
 * @param {string<Array>} pathParams array of string that represent some part of the api/URL
 * @return {string}
 *
 */

export const parsePathParams = (pathParams) => {
	if (!pathParams || !isArray(pathParams)) return '';

	return pathParams.filter((param) => !!param && isString(param)).join('/');
};

/***
 * @name URLMaker
 * @description funciton to make URL
 * @param {object} params
 * @param {string} params.queryParams Filters and sorts. Will be parsed and added to the url.
 ...
 * @param {string} params.pathParams string that represent some part of the api/URL
...
export const URLMaker = ({
	...
	const extendedURL = [namespace, pathParams, id, action]
		.map((value) => (!!value && isString(value) ? `/${value}` : ''))
		.join('');

	if (!extendedURL) return '';

	return `${getApiService(service, environment)}${extendedURL}${queryParams}`;
};
